...
function buildNavItems(params: {
  ...
  const items: NavItem[] = [
    ...
    { href: "/studies", label: "Casos clínicos", icon: FolderOpen },
  ];
  ...
  if (canAccessClinicMenu) {
    items.push({ href: "/clinic", label: "Clínica", icon: Building2 });
  }

  const canSeeIndividualBilling = effectiveRole === "individual" && !hasClinic;
  if (canSeeIndividualBilling) {
    items.push({ href: "/billing", label: "Assinatura", icon: CreditCard });
  }
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 break-words text-sm text-slate-400">
                        {user?.clinic_name || "Nenhuma clínica vinculada"}
                      </p>
                    </div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {user?.clinic_name || "Espaço individual"}
                </p>
              </div>
